...
class $modify(BRBaseGameLayer, GJBaseGameLayer) {
    ...
    void handleButton(bool down, int button, bool isPlayer1) {
        ...
        if (BotReplay::get()->isRecording()) {
            ...
            // Small on-screen confirmation the first few times, so it's
            // visible even without checking logs.
            if (BotReplay::get()->frameCount() <= 5) {
                Notification::create(
                    fmt::format("Captured input #{}", BotReplay::get()->frameCount()).c_str(),
