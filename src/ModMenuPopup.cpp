...
void ModMenuPopup::onRecordRadio(CCObject*) {
    ...
    // Restart the attempt so frame numbers line up with the start of the
    // level (frame 0) - this is how real macro tools like xdBot do it.
    // Recording mid-run (after you've already progressed into the level)
    // means the frame numbers won't correspond to the same obstacles when
    // played back later.
    pl->resetLevel();
    br->startRecording();
    this->removeFromParentAndCleanup(true);
    Notification::create(
        "Level restarted - resume now to record from the start!",
        NotificationIcon::Success
    )->show();
}
...
void ModMenuPopup::onPlayRadio(CCObject*) {
    ...
    // Same reasoning as Record: restart the attempt so the replay's
    // frame-0-aligned inputs match up with the level from its start.
    pl->resetLevel();
    br->startPlaying();
    this->removeFromParentAndCleanup(true);
    Notification::create(
        "Level restarted - resume now to play back the replay!",
        NotificationIcon::Success
    )->show();
}
...
void ModMenuPopup::buildBotReplayTab() {
    ...
    auto tip = CCLabelBMFont::create(
        "Pause the level, tap Record, then resume\nto capture your run. Pause again to Stop.",
        "chatFont.fnt"
    );
    tip->setScale(0.38f);
