...
void BotReplay::startRecording() {
    ...
    m_recording = true;
    m_playing = false;
    m_recordOriginFrame = 0; // set on the first captured input, see onInput
}
...
void BotReplay::onInput(int button, bool player2, bool down, int frame) {
    ...
    if (m_frames.empty()) {
        // First captured input becomes frame 0 - everything else is
        // stored relative to this moment.
